...
insert_node (Node pknode children) config node = are_we_done $ insert_into_children check_instantiable config children node
	...
	populate_subdivisions = foldr (\child subdivisions -> insert_into_children return config subdivisions child) $ map build_node_from_subdivision $ (pk_zone pknode) `divide_zone_by` (config_r config)
